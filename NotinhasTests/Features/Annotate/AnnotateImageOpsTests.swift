...
final class AnnotateImageOpsTests: XCTestCase {
  ...
  func testImportImageWithExistingBaseAppendsEmbeddedLayer() throws {
    ...
    let accepted = try state.importImage(makeImage(width: 120, height: 90))
  ...
  func testMarkupToolsTreatCombinedImageLayersAsCanvas() {
  ...
  func testAddImportedImageWithZeroSizedImageIsIgnored() throws {
    let state = makeAnnotateState()
    try state.loadImage(makeImage(width: 200, height: 200))
  ...
  func testAddImportedImageAppendsValidLayer() throws {
    ...
    try state.addImportedImage(makeImage(width: 60, height: 60))
  ...
  func testImportOversizedImageIsAcceptedNotRejected() throws {
    ...
    let accepted = try state.importImage(makeImage(width: 4000, height: 4000))
