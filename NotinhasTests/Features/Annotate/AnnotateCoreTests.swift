...
final class AnnotateCoreTests: XCTestCase {
  ...
  func testSensitiveRedactionContextDetectsPaymentCardFieldsSplitIntoFourOCRFragments() {
    ...
    XCTAssertFalse(regions
      .contains { $0.bounds.intersects(CGRect(x: 189, y: 282.7, width: 21, height: 14)) && $0.kind != .creditCard })
  }
  ...
  func testAnnotateExporterGenerateCopyURL_incrementsExistingCopies() throws {
    let directory = FileManager.default.temporaryDirectory
      .appendingPathComponent("NotinhasTests_AnnotateCopyURL_\(UUID().uuidString)", isDirectory: true)
  ...
  func testCodableBackgroundStyle_roundTripsSupportedStyles() throws {
    let wallpaperURL = try XCTUnwrap(URL(string: "file:///tmp/wallpaper.jpg"))
    let blurredURL = try XCTUnwrap(URL(string: "file:///tmp/blurred.jpg"))
