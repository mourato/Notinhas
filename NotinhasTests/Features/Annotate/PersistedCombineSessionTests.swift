...
final class PersistedCombineSessionTests: XCTestCase {
  ...
  override func setUp() {
    ...
    tempDirectory = FileManager.default.temporaryDirectory
      .appendingPathComponent("NotinhasTests_CombineSession_\(UUID().uuidString)", isDirectory: true)
  ...
  func testPersistAndLoad_roundTripsCombineSession() throws {
    ...
    let layerId = try XCTUnwrap(UUID(uuidString: "AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA"))
  ...
  func testToSnapshot_dropsUnparseableUUIDKeys() throws {
  ...
  private func makeSessionData(combineSession: CombineSessionSnapshot?) throws -> AnnotationSessionData {
    try AnnotationSessionData(
      originalImageData: makeImageData(width: 24, height: 16),
      annotations: [],
