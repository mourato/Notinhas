...
final class PostCaptureActionHandlerTests: XCTestCase {
  ...
  override func setUp() async throws {
    ...
    tempDirectory = FileManager.default.temporaryDirectory
      .appendingPathComponent("NotinhasTests_PostCapture_\(UUID().uuidString)", isDirectory: true)
  ...
  private func saveDefaultPreset(
    padding: CGFloat = 10,
    backgroundStyle: Notinhas.BackgroundStyle = .solidColor(SwiftUI.Color.red)
  ...
  func testHandleScreenshotCaptures_filtersMissingFiles() async {
    preferences.setAction(.copyFile, for: .screenshot, enabled: false)
