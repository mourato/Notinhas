...
final class CloudCoreTests: XCTestCase {
  ...
  func testAWSV4SignerSign_addsRequiredHeadersAndAuthorizationScope() throws {
    var request =
      URLRequest(
        url: try XCTUnwrap(
          URL(string: "https://example-bucket.s3.us-east-1.amazonaws.com/snapzy/test.png?acl=&z=last&a=first")
        )
      )
  ...
  func testAWSV4SignerPresignURL_includesExpectedQueryItems() throws {
    let presignedURL = try AWSV4Signer.presignURL(
      url: XCTUnwrap(URL(string: "https://example-bucket.s3.us-east-1.amazonaws.com/snapzy/test.png")),
      accessKey: "AKIATEST",
      secretKey: "SECRET",
      region: "us-east-1",
      expireSeconds: 900
  ...
  func testCloudCredentialTransferSuggestedArchiveFileName_sanitizesBucket() {
    ...
    XCTAssertEqual(
      ...
      "notinhas-cloud-cloudflare-r2-My-Bucket-2026.notinhascloud"
    )
  }
