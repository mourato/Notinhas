//
//  NotinhasConfigurationAutoImporterTests.swift
//  NotinhasTests
//
...
@MainActor
final class NotinhasConfigurationAutoImporterTests: XCTestCase {
  private var tempDirectory: URL!

  override func setUpWithError() throws {
    try super.setUpWithError()
    tempDirectory = FileManager.default.temporaryDirectory
      .appendingPathComponent("NotinhasConfigurationAutoImporterTests-\(UUID().uuidString)", isDirectory: true)
  ...
  func testAutoImportAppliesChangedConfigFileAndStoresSignature() throws {
    ...
    let result = NotinhasConfigurationAutoImporter.applyIfNeeded(from: fileURL, defaults: defaults)
    ...
    let secondResult = NotinhasConfigurationAutoImporter.applyIfNeeded(from: fileURL, defaults: defaults)
