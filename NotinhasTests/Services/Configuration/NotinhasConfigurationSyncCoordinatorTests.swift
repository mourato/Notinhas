//
//  NotinhasConfigurationSyncCoordinatorTests.swift
//  NotinhasTests
//
...
@MainActor
final class NotinhasConfigurationSyncCoordinatorTests: XCTestCase {
  func testScheduleSyncDebouncesRepeatedRequests() async throws {
    ...
    let coordinator = makeCoordinator(
      ...
      sync: {
        syncCount += 1
        return NotinhasConfigurationSyncResult(
          status: .synced,
          fileURL: fileURL,
  ...
  func testSyncNowReportsConflictWithoutConfirmedOverwrite() throws {
    ...
    let coordinator = makeCoordinator(
      sync: {
        syncCount += 1
        return NotinhasConfigurationSyncResult(status: .needsConfirmation, fileURL: fileURL)
      },
  ...
  func testPermissionRequiredStatusIsExposed() throws {
    ...
    let coordinator = makeCoordinator(
      sync: {
        NotinhasConfigurationSyncResult(status: .permissionRequired, fileURL: fileURL)
      }
    )
    defer { coordinator.stop() }
  ...
  private func makeCoordinator(
    ...
    sync: @escaping @MainActor () throws -> NotinhasConfigurationSyncResult = {
      NotinhasConfigurationSyncResult(status: .alreadyCurrent, fileURL: URL(fileURLWithPath: "/tmp/config.toml"))
    },
    backgroundSync: (@MainActor () async throws -> NotinhasConfigurationSyncResult)? = nil,
  ...
  ) -> NotinhasConfigurationSyncCoordinator {
    NotinhasConfigurationSyncCoordinator(
      notificationCenter: notificationCenter,
