//
//  NotinhasConfigurationServiceTests.swift
//  NotinhasTests
//
...
@MainActor
final class NotinhasConfigurationServiceTests: XCTestCase {
  func testConfigFileURLAppendsConfigTomlToSelectedDirectory() {
    ...
    let url = NotinhasConfigurationService.shared.configFileURL(inDirectory: directory)
  ...
  func testSuggestedConfigDirectoryMatchingUsesCanonicalPath() {
    let expectedDirectory = NotinhasConfigurationPaths.suggestedConfigDirectoryURL

    XCTAssertTrue(NotinhasConfigurationService.shared.isSuggestedConfigDirectory(expectedDirectory))
    XCTAssertFalse(
      NotinhasConfigurationService.shared.isSuggestedConfigDirectory(
  ...
  func testSuggestedConfigParentDirectoryMatchingUsesCanonicalPath() {
    let expectedParentDirectory = NotinhasConfigurationPaths.suggestedConfigDirectoryURL
      .deletingLastPathComponent()

    XCTAssertTrue(NotinhasConfigurationService.shared.isSuggestedConfigParentDirectory(expectedParentDirectory))
    XCTAssertFalse(
      NotinhasConfigurationService.shared.isSuggestedConfigParentDirectory(
  ...
  func testSuggestedConfigRootDirectoryMatchingUsesCanonicalPath() {
    let expectedRootDirectory = NotinhasConfigurationPaths.userHomeDirectory

    XCTAssertTrue(NotinhasConfigurationService.shared.isSuggestedConfigRootDirectory(expectedRootDirectory))
    XCTAssertFalse(
      NotinhasConfigurationService.shared.isSuggestedConfigRootDirectory(
  ...
  func testEnsureConfigExistsCreatesParentDirectoryAndFile() throws {
    ...
    let url = NotinhasConfigurationPaths.suggestedConfigURL(homeDirectory: homeDirectory)

    let returnedURL = try NotinhasConfigurationService.shared.ensureConfigExists(at: url)
  ...
  func testExportIncludesQuickAccessTwoFingerSwipeSetting() throws {
    ...
    let source = NotinhasConfigurationExporter.exportTOML(defaults: UserDefaultsFactory.make())
    let document = try SimpleTOMLParser.parse(source)
  ...
  func testEnsureConfigExistsDoesNotOverwriteExistingFile() throws {
    ...
    try NotinhasConfigurationService.shared.ensureConfigExists(at: url)
  ...
  func testImportBackupReplacingManagedConfigWritesSelectedTomlToManagedFile() throws {
    ...
    let result = try NotinhasConfigurationService.shared.importBackupReplacingManagedConfig(
      from: backupURL,
  ...
  func testSyncDecisionAlreadyCurrentWhenSourcesMatch() {
    ...
    let decision = NotinhasConfigurationService.syncDecision(
      fileSource: source,
  ...
  func testSyncDecisionAutoSyncsWhenFileMatchesLastAppliedSignature() {
    ...
    NotinhasConfigurationAutoImporter.markCurrentFileApplied(fileSource, defaults: defaults)
  ...
  func testPrepareManagedConfigForOpeningCreatesMissingFileFromCurrentSettings() throws {
    try withRestoredLastAppliedSignature {
      ...
      let result = try NotinhasConfigurationService.shared.prepareManagedConfigForOpening(at: managedURL)

      XCTAssertEqual(result.status, .synced)
      XCTAssertTrue(FileManager.default.fileExists(atPath: managedURL.path))
      let source = try String(contentsOf: managedURL, encoding: .utf8)
      XCTAssertTrue(NotinhasConfigurationAutoImporter.isCurrentFileApplied(source))
    }
  }
  ...
  func testPrepareManagedConfigForOpeningAutoSyncsStaleAppOwnedFile() throws {
    try withRestoredLastAppliedSignature {
      ...
      NotinhasConfigurationAutoImporter.markCurrentFileApplied(staleSource)
      ...
      XCTAssertTrue(NotinhasConfigurationAutoImporter.isCurrentFileApplied(syncedSource))
    }
  }
  ...
  func testSyncManagedConfigToCurrentSettingsOverwritesAfterConfirmation() throws {
    try withRestoredLastAppliedSignature {
      ...
      try NotinhasConfigurationService.shared.syncManagedConfigToCurrentSettings(at: managedURL)
      let syncedSource = try String(contentsOf: managedURL, encoding: .utf8)
  ...
  func testSyncManagedConfigToCurrentSettingsIfUnchangedOverwritesApprovedFile() throws {
    try withRestoredLastAppliedSignature {
      ...
      let result = try NotinhasConfigurationService.shared.prepareManagedConfigForOpening(at: managedURL)
      try NotinhasConfigurationService.shared.syncManagedConfigToCurrentSettingsIfUnchanged(
        at: managedURL,
  ...
  func testSyncManagedConfigToCurrentSettingsIfUnchangedDoesNotOverwriteChangedFile() throws {
    try withRestoredLastAppliedSignature {
      ...
      XCTAssertThrowsError(
        ...
        XCTAssertTrue(error is NotinhasConfigurationSyncError)
      }
