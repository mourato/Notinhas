...
struct AppLaunchPolicy {
  ...
  private var allowsInteractiveXCTestHost: Bool {
    environment["NOTINHAS_ALLOW_INTERACTIVE_XCTEST_HOST"] == "1"
  }
}
...
class AppDelegate: NSObject, NSApplicationDelegate {
  ...
  // MARK: - Open With (Finder right-click → Open With → Notinhas)

  /// Called when the user opens one or more image files with Notinhas from
  /// Finder's "Open With" submenu, by double-clicking a file whose default
  /// app is Notinhas, or by drag-dropping files onto the app icon in the Dock.
  ///
