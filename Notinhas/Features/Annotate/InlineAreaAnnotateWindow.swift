...
enum InlineAreaLayout {
  static let toolbarHeight: CGFloat = InlineAreaToolbarMetrics.iconButtonSize + InlineAreaToolbarMetrics
    .verticalPadding * 2
...
enum InlineAreaControlGeometry {
  ...
  private static func actionRailPosition(
    ...
    let x: CGFloat = if rightX <= maximumX {
      rightX
...
private struct InlineAreaPaletteColorButton: View {
  ...
  var onDropPayload: ((AnnotateColorDragPayload) -> Void)?
  let onSelect: () -> Void
  ...
  var body: some View {
    HStack(spacing: 6) {
      Image(systemName: isTargeted ? "arrow.down" : "plus")
        ...
        .frame(width: 24, height: 24)
        .background(Circle()
          .fill(isTargeted ? Color.accentColor.opacity(0.1) : InlineAreaChrome.itemBackground.opacity(0.75)))
        .overlay(
          Circle()
            .stroke(
        ...
        ForEach(items) { item in
          Button {
            ...
            Image(systemName: icon(item))
              ...
              .foregroundColor(selection == item ? InlineAreaChrome.itemSelectedForeground : InlineAreaChrome
                .secondaryText)
              ...
              .overlay(
                RoundedRectangle(cornerRadius: InlineAreaChrome.controlCornerRadius, style: .continuous)
                  .stroke(
                    selection == item ? InlineAreaChrome.itemSelectedBorder : InlineAreaChrome.itemBorder,
                    lineWidth: 1
                  )
              )
          }
    ...
    InlineAreaPropertyGroup(title: L10n.AnnotateUI.arrowBend) {
      Button {
        ...
        Image(systemName: bendDirection.icon)
          .font(.system(size: 12, weight: .medium))
          .foregroundColor(bendDirection == .alternate ? InlineAreaChrome.itemSelectedForeground : InlineAreaChrome
            .secondaryText)
          .frame(width: 25, height: InlineAreaChrome.propertyControlHeight)
          .background(
            RoundedRectangle(cornerRadius: InlineAreaChrome.controlCornerRadius, style: .continuous)
              .fill(bendDirection == .alternate ? InlineAreaChrome.itemSelectedBackground : InlineAreaChrome
                .itemBackground)
          )
          .overlay(
            RoundedRectangle(cornerRadius: InlineAreaChrome.controlCornerRadius, style: .continuous)
              .stroke(
                bendDirection == .alternate ? InlineAreaChrome.itemSelectedBorder : InlineAreaChrome.itemBorder,
                lineWidth: 1
              )
          )
      }
