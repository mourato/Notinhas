...
enum AnnotateSensitiveRedactionError: LocalizedError {
  ...
  var errorDescription: String? {
    switch self {
    case .imageUnavailable:
      "Unable to load image data for sensitive-data detection."
    case .textRecognitionFailed:
      "Unable to recognize text for sensitive-data detection."
    }
  }
}
...
final class AnnotateSensitiveRedactionService {
  ...
  private static func looksLikeCardholderName(_ text: String) -> Bool {
    let excluded: Set = ["BANK", "VALID", "THRU", "MEMBER", "SINCE", "CARD", "NAME"]
    let tokens = text
      .split(separator: " ")
      .map(String.init)
      .filter { !$0.isEmpty }
