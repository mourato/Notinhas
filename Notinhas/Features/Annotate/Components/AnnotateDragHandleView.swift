...
struct AnnotateDragHandleView: NSViewRepresentable {
  ...
  func makeNSView(context _: Context) -> DragHandleNSView {
    let view = DragHandleNSView(state: state)
    view.updateState(state)
    return view
  }

  func updateNSView(_ nsView: DragHandleNSView, context _: Context) {
...
extension DragHandleNSView {
  ...
  private func scheduleFallbackPreparationIfNeeded() {
    ...
    fallbackPreparationTask = Task(priority: .utility) { @MainActor [weak self] in
      ...
      guard let self,
            !Task.isCancelled,
            fallbackGenerationSequence == generation,
            currentFallbackSignature == signature else { return }
      prepareConcreteFallbackFileIfNeeded(for: signature)
    }
  }
