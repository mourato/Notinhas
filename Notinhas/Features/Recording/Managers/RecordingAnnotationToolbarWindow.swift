...
//
  ...
  private class PopoverArrowView: NSView {
    ...
    override func draw(_: NSRect) {
      ...
      case .bottom:
        NSRect(x: 0, y: arrowHeight, width: bounds.width, height: bounds.height - arrowHeight)
      }
  ...
  final class RecordingAnnotationToolbarWindow: NSWindow {
    ...
    private func rebuildContent() {
      ...
      let effectOriginY: CGFloat = switch arrowEdge {
      case .top:
        0
      case .bottom:
        arrowHeight
    ...
    func positionRelativeToAnchor() {
      ...
      // Compute Y
      let popoverY: CGFloat = switch arrowEdge {
      case .top:
        // Popover below status bar: arrow points up toward the bar
        anchorFrame.origin.y - popoverSize.height - popoverGap
      case .bottom:
        // Popover above status bar: arrow points down toward the bar
        anchorFrame.maxY + popoverGap
      }
