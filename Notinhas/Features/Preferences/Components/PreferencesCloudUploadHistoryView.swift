...
enum HistoryStatusFilter: String, CaseIterable {
  ...
  var label: String {
    switch self {
    case .all: L10n.PreferencesCloudHistory.statusAll
    case .active: L10n.PreferencesCloudHistory.statusActive
    case .expired: L10n.PreferencesCloudHistory.statusExpired
    }
  }
}
...
enum HistorySortOrder: String, CaseIterable {
  ...
  var label: String {
    switch self {
    case .newestFirst: L10n.PreferencesCloudHistory.newestFirst
    case .oldestFirst: L10n.PreferencesCloudHistory.oldestFirst
    case .largestFirst: L10n.PreferencesCloudHistory.largestFirst
    case .smallestFirst: L10n.PreferencesCloudHistory.smallestFirst
    }
  }
}
...
struct CloudUploadHistoryView: View {
  ...
  private var filteredRecords: [CloudUploadRecord] {
    ...
    case .expired: result = result.filter(\.isExpired)
    }
  ...
  private var errorBanner: some View {
    if let error = deleteError {
      ...
      .background(
        Color.orange.opacity(colorScheme == .dark ? 0.18 : 0.11),
