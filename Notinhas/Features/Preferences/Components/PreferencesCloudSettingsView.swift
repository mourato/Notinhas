...
private enum CloudProtectedAction {
  ...
  var passwordPrompt: String {
    switch self {
    case .editCredentials:
      L10n.CloudSettings.editCredentialsPasswordPrompt
    case .importCredentials:
...
struct CloudSettingsView: View {
      ...
      if let config = cloudManager.cachedConfiguration {
        if config.providerType == .googleDrive {
          SettingRow(
            ...
            description: config.bucket.isEmpty ? "Notinhas" : config.bucket
          ) {
            EmptyView()
          }
  ...
  private func lifecycleShortLabel(_ days: Int?) -> String {
    guard let days else { return L10n.CloudUsage.none }
...
private struct CloudCredentialFormView: View {
  ...
  @State private var googleFolderName = "Notinhas"
      ...
      if providerType == .googleDrive {
          ...
          HStack(spacing: 12) {
            Button(action: startGoogleAuthorization) {
              HStack(spacing: 6) {
                ...
                Text(isAuthorizing ? L10n.CloudSettings
                  .googleAuthorizing :
                  (authorizationSuccess ? L10n.CloudSettings.googleAuthorized : L10n.CloudSettings.googleAuthorize))
              }
            }
            .disabled(googleClientId.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || googleClientSecret
              .trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || isAuthorizing)
        ...
        Section(L10n.CloudSettings.googleFolderSection) {
          SettingRow(
            ...
            TextField("Notinhas", text: $googleFolderName)
              .textFieldStyle(.roundedBorder)
              .frame(width: 240)
          }
  ...
  private func saveAndTest() {
    ...
    let config = CloudConfiguration(
      ...
      bucket: isGoogle ? googleFolderName.trimmingCharacters(in: .whitespaces) : bucket
        .trimmingCharacters(in: .whitespaces),
      region: isGoogle ? "" : region.trimmingCharacters(in: .whitespaces),
      endpoint: isGoogle ? nil :
        (endpoint.trimmingCharacters(in: .whitespaces).isEmpty ? nil : endpoint.trimmingCharacters(in: .whitespaces)),
      customDomain: isGoogle ? nil :
        (customDomain.trimmingCharacters(in: .whitespaces).isEmpty ? nil : customDomain
          .trimmingCharacters(in: .whitespaces)),
      expireTime: isGoogle ? .permanent : expireTime
    )

    Task {
      let trimmedAccessKey = isGoogle ? googleClientId.trimmingCharacters(in: .whitespacesAndNewlines) : accessKey
        .trimmingCharacters(in: .whitespacesAndNewlines)
      let trimmedSecretKey = isGoogle ? googleClientSecret.trimmingCharacters(in: .whitespacesAndNewlines) : secretKey
        .trimmingCharacters(in: .whitespacesAndNewlines)
