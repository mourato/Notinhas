...
/// Manages detection and resolution of conflicts between active Notinhas shortcuts
...
final class SystemScreenshotShortcutManager {
  ...
  private enum SystemHotkeyID: Int, CaseIterable {
    ...
    var displayName: String {
      switch self {
      case .saveAreaToFile:
        L10n.SystemShortcuts.macOSCaptureArea
      case .copyAreaToClipboard:
        L10n.SystemShortcuts.macOSCopyArea
      case .saveScreenToFile:
        L10n.SystemShortcuts.macOSCaptureFullscreen
      case .copyScreenToClipboard:
        L10n.SystemShortcuts.macOSCopyFullscreen
      case .screenshotOptions:
        L10n.SystemShortcuts.macOSScreenshotOptions
      }
    }
  }
  ...
  /// currently-enabled Notinhas fullscreen/area/recording shortcuts.
  ///
  ...
  /// Return human-readable system shortcut names that currently conflict with a proposed Notinhas shortcut.
  ...
  private func relevantSystemHotkeys(for kind: GlobalShortcutKind) -> [SystemHotkeyID] {
    switch kind {
    case .fullscreen:
      [.saveScreenToFile, .copyScreenToClipboard]
    case .area:
      [.saveAreaToFile, .copyAreaToClipboard]
    case .recording:
      [.screenshotOptions]
    default:
      []
