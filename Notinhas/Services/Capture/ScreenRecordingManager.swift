...
//
  ...
  enum RecordingAudioCompatibilityExporter {
    ...
    private static func writeNormalizedFile(
      ...
      try await withCheckedThrowingContinuation { continuation in
        let workerQueue = DispatchQueue(label: "com.mourato.notinhas.recording.audio-compatibility", qos: .utility)
        workerQueue.async {
          do {
    ...
    private static func copySamples(
      ...
      for (label, output, input) in outputsAndInputs {
        group.enter()
        let queue = DispatchQueue(label: "com.mourato.notinhas.recording.audio-compatibility.\(label)")
        var didFinish = false
  ...
  final class ScreenRecordingManager: NSObject, ObservableObject {
    ...
    func prepareRecording(
      ...
      captureWindowTarget = windowTarget
