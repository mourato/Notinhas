...
private let logger = Logger(subsystem: "Notinhas", category: "CloudUsageService")
...
private actor CloudUsageWorker {
  ...
  private func makeRequestContext(config: CloudConfiguration) -> RequestContext {
    ...
    let endpoint: String = if let configuredEndpoint = config.endpoint?.trimmingCharacters(in: .whitespacesAndNewlines),
...
final class CloudUsageService: ObservableObject {
  ...
  func fetchUsage(forceRefresh: Bool = false) async {
    ...
    let task = Task { [weak self] in
      ...
      do {
        let info = try await worker.fetchUsage(
          config: config,
...
/// Parses lifecycle configuration XML to find the Notinhas auto-expire rule.
