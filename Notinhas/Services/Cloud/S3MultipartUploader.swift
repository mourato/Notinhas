...
private let logger = Logger(subsystem: "Notinhas", category: "S3MultipartUploader")
      ...
      for partNumber in 1 ... totalParts {
        ...
        while retryCount < 3, partETag == nil {
          do {
            ...
            retryCount += 1
            logger
              .warning("Failed to upload part \(partNumber) (attempt \(retryCount)/3): \(error.localizedDescription)")
            if retryCount >= 3 {
              throw error
            }
            // Exponential backoff
