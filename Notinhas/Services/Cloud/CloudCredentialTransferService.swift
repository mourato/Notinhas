...
enum CloudCredentialTransferError: LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case .passphraseTooShort(let minimumLength):
      L10n.CloudTransfer.exportPassphraseTooShort(minimumLength)
    case .invalidArchive:
      L10n.CloudTransfer.invalidArchive
    case .unsupportedSchemaVersion(let version):
      L10n.CloudTransfer.unsupportedSchemaVersion(version)
    case .unsupportedArchiveFormat:
      L10n.CloudTransfer.unsupportedArchiveFormat
    case .unlockFailed:
      L10n.CloudTransfer.unlockFailed
    case .randomizationFailed:
      L10n.CloudTransfer.randomizationFailed
    }
  }
}

enum CloudCredentialTransferService {
  static let archiveFileExtension = "notinhascloud"
  static let legacyArchiveFileExtension = "snapzycloud"
  static let supportedArchiveFileExtensions = [archiveFileExtension, legacyArchiveFileExtension]
  ...
  static func suggestedArchiveFileName(for payload: CloudCredentialTransferPayload) -> String {
    ...
    return "notinhas-cloud-\(provider)-\(bucket).\(archiveFileExtension)"
  }
