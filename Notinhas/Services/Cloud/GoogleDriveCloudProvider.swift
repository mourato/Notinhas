...
private let logger = Logger(subsystem: "Notinhas", category: "GoogleDriveCloudProvider")
...
final class GoogleDriveCloudProvider: CloudProvider {
  ...
  init(
    ...
    self.folderName = folderName.isEmpty ? "Notinhas" : folderName
  ...
  func generatePublicURL(for key: String) -> URL {
    ...
    URL(string: "https://drive.google.com/file/d/\(key)/view")!
  }
    ...
    while bytesUploaded < fileSize {
      ...
      if bytesUploaded == fileSize {
        guard httpResponse.statusCode == 200 || httpResponse.statusCode == 201 else {
          ...
          throw CloudError.uploadFailed(
            statusCode: httpResponse.statusCode,
            message: "Resumable upload final chunk failed: \(body)"
          )
        }
        ...
        guard httpResponse.statusCode == 308 else {
          ...
          throw CloudError.uploadFailed(
            statusCode: httpResponse.statusCode,
            message: "Resumable upload chunk failed: \(body)"
          )
        }
      }
    }
