...
final class CloudPasswordService {
  ...
  private let logger = Logger(subsystem: "Notinhas", category: "CloudPasswordService")
  ...
  func verifyPassword(_ password: String) -> CloudPasswordVerificationResult {
    ...
    case .authRequired:
      DiagnosticLogger.shared.log(
        .warning,
        .cloud,
        "Cloud credential password verification requires keychain authentication"
      )
    ...
    case .interactionNotAllowed:
      DiagnosticLogger.shared.log(
        .warning,
        .cloud,
        "Cloud credential password verification keychain interaction unavailable"
      )
