...
enum CloudProviderType: String, Codable, CaseIterable {
  ...
  var displayName: String {
    switch self {
    case .awsS3: L10n.CloudProvider.awsS3
    case .cloudflareR2: L10n.CloudProvider.cloudflareR2
    case .googleDrive: L10n.CloudProvider.googleDrive
    }
  }
}
...
protocol CloudProvider {
  ...
  /// Remove the Notinhas lifecycle expiration rule from the bucket.
...
enum CloudError: LocalizedError {
  ...
  var errorDescription: String? {
    switch self {
    case .notConfigured:
      L10n.CloudOperation.notConfigured
    case .invalidCredentials:
      L10n.CloudOperation.invalidCredentials
    case .uploadFailed(let code, let message):
      L10n.CloudOperation.uploadFailed(code, message: message)
    case .networkError(let error):
      L10n.CloudOperation.networkError(error.localizedDescription)
    case .fileNotFound(let url):
      L10n.CloudOperation.fileNotFound(url.lastPathComponent)
    case .signingFailed(let reason):
      L10n.CloudOperation.signingFailed(reason)
    case .invalidResponse:
      L10n.CloudOperation.invalidResponse
