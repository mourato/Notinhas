...
private let logger = Logger(subsystem: "Notinhas", category: "GoogleDriveOAuthService")
...
final class GoogleDriveOAuthService: @unchecked Sendable {
  ...
  private let queue = DispatchQueue(label: "com.mourato.notinhas.oauth")
    ...
    do {
      code = try await withCheckedThrowingContinuation { continuation in
        ...
        components.queryItems = [
          ...
          URLQueryItem(
            name: "scope",
            value: "https://www.googleapis.com/auth/drive.file https://www.googleapis.com/auth/userinfo.email"
          ),
  ...
  func refreshAccessToken(
    ...
    guard httpResponse.statusCode == 200 else {
      ...
      throw CloudError.uploadFailed(
        ...
        message: "Token refresh failed: \(responseString)"
      )
    }
  ...
  private func receive(on connection: NWConnection) {
    connection.receive(minimumIncompleteLength: 1, maximumLength: 8192) { [weak self] content, _, _, error in
      ...
      if let requestString = String(data: content, encoding: .utf8) {
        parseHTTPRequest(requestString, connection: connection)
      } else {
        closeConnection(connection)
      }
    }
  ...
  private func sendErrorResponse(error: String, connection: NWConnection) {
    ...
    <body>
      <div class="container">
        ...
        <p>Google Drive authorization failed with error: <strong>\(
          error
        )</strong>. Please return to Notinhas and try again.</p>
        <div class="action-btn-container">
          <a href="notinhas://settings/cloud" class="btn">Return to Notinhas</a>
        </div>
      </div>
  ...
  private func exchangeCodeForTokens(
    ...
    guard httpResponse.statusCode == 200 else {
      ...
      throw CloudError.uploadFailed(
        statusCode: httpResponse.statusCode,
        message: "Token exchange failed: \(responseString)"
      )
    }
