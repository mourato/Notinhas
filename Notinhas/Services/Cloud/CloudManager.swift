...
private let logger = Logger(subsystem: "Notinhas", category: "CloudManager")
...
final class CloudManager: ObservableObject {
  ...
  func saveConfiguration(
    ...
    let googleRefreshTokenSnapshot = isGoogle ? snapshotCredential(
      item: .googleRefreshToken,
      context: "saveConfiguration.snapshot.googleRefreshToken"
    ) : nil
  ...
  func maskedEndpoint() -> String {
    ...
    if hostParts.count >= 2 {
      ...
      let maskedSub = if subdomain.count > 8 {
        "\(subdomain.prefix(4))••••\(subdomain.suffix(4))"
      } else {
        "••••••••"
      }
  ...
  private func rollbackCredentialWrite(
    ...
    if let googleRefreshTokenSnapshot {
      restoreCredential(
        item: .googleRefreshToken,
        snapshot: googleRefreshTokenSnapshot,
        context: "saveConfiguration.rollback.googleRefreshToken"
      )
    }
  }
