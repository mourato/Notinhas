//
//  NotinhasConfigurationSyncCoordinator.swift
//  Notinhas
//
...
@MainActor
final class NotinhasConfigurationSyncCoordinator: ObservableObject {
  ...
  static let shared = NotinhasConfigurationSyncCoordinator()

  @Published private(set) var status: Status = .idle

  private let notificationCenter: NotificationCenter
  private let debounceInterval: TimeInterval
  private let syncManagedConfigIfSafe: @MainActor () throws -> NotinhasConfigurationSyncResult
  private let syncManagedConfigIfSafeInBackground: @MainActor () async throws -> NotinhasConfigurationSyncResult
  ...
  init(
    ...
    syncManagedConfigIfSafe = {
      try service.syncManagedConfigIfSafe()
    }
    syncManagedConfigIfSafeInBackground = {
    ...
    currentSettingsSignature = {
      NotinhasConfigurationAutoImporter.contentSignature(for: service.exportTOML())
    }
  }
  ...
  convenience init(
    ...
    self.init(
      service: NotinhasConfigurationService.shared,
  ...
  init(
    ...
    syncManagedConfigIfSafe: @escaping @MainActor () throws -> NotinhasConfigurationSyncResult,
    syncManagedConfigIfSafeInBackground: (@MainActor () async throws -> NotinhasConfigurationSyncResult)? = nil,
    ...
    syncCurrentSettings = syncCurrentSettingsAfterConfirmation
  ...
  func scheduleSync(reason: Reason) {
    ...
    debounceTask = Task { @MainActor [weak self] in
      ...
      debounceTask = nil
      do {
        try await syncNowInBackground(reason: reason)
      } catch {
  ...
  @discardableResult
  func flushPendingSync(reason: Reason) throws -> NotinhasConfigurationSyncResult? {
  ...
  @discardableResult
  func syncNow(reason: Reason = .manual) throws -> NotinhasConfigurationSyncResult {
    cancelPendingSync()
    let sequence = beginSync()
    var syncResult: NotinhasConfigurationSyncResult?
    defer {
  ...
  @discardableResult
  func syncNowInBackground(reason: Reason = .manual) async throws -> NotinhasConfigurationSyncResult {
    cancelPendingSync()
    let sequence = beginSync()
  ...
  private func updateStatus(for result: NotinhasConfigurationSyncResult, sequence: Int) {
  ...
  private func finishSyncing(after result: NotinhasConfigurationSyncResult?, sequence _: Int) {
  ...
  private func log(_ result: NotinhasConfigurationSyncResult, reason: Reason) {
    DiagnosticLogger.shared.log(
      .debug,
      .preferences,
