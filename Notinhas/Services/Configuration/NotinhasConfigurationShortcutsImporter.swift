//
//  NotinhasConfigurationShortcutsImporter.swift
//  Notinhas
//
...
extension NotinhasConfigurationImporter {
  ...
  private static func collectQuickAccessShortcut(
    ...
    if let key {
      guard let shortcut = NotinhasConfigurationShortcutCodec.shortcut(
        key: key,
        modifiers: modifiers ?? [],
  ...
  static func quickAccessSlots(
    from reader: inout NotinhasConfigurationReader
  ...
  private static func collectOverlayShortcut(
    ...
    guard let key,
          let shortcut = NotinhasConfigurationShortcutCodec.overlayShortcut(
            key: key,
            modifiers: modifiers ?? []
          ) else {
    ...
    for tool in AnnotateShortcutManager.configurableTools {
      if let key = reader.string("shortcuts", "annotate_tools", tool.rawValue) {
        ...
        guard normalizedKey.count == 1,
          ...
          reader
            .error("shortcuts.annotate_tools.\(tool.rawValue) must be a single letter, number, or special character")
          continue
        }
