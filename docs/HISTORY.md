...
- `HistoryFloatingManager` — panel state. Position `topCenter` / `bottomCenter` (`HistoryPanelPosition.center` exists for config import, not in UI); panel width is 90% of the active display's visible area, height follows the hosted SwiftUI content and is clamped to the visible display, and the top position uses an 8pt top margin; background `HistoryBackgroundStyle` hud / solid.
